...
if ( ! class_exists( 'ESIG_WOOCOMMERCE_Admin' ) ) :
	...
	class ESIG_WOOCOMMERCE_Admin {
		...
		public function woo_init() {
			...
			// Prevent esig_product_agreement (stored as an array) from being
			// surfaced by third-party cart plugins that blindly iterate all cart
			// item keys via woocommerce_get_item_data. Running at 9999 ensures
			// we strip it after any such plugin has added it.
			add_filter( 'woocommerce_get_item_data', array( $this, 'hide_agreement_from_cart_display' ), 9999, 2 );
		...
		/**
		 * Remove esig_product_agreement from the cart item data shown to customers.
		 *
		 * esig_product_agreement is an internal array used to track signing state.
		 * It must never be rendered as a display value: on PHP 8.x, passing an array
		 * through wpautop() → trim() raises a TypeError. This filter runs at priority
		 * 9999 so it strips the entry even when a third-party cart plugin (e.g.
		 * side-cart-woocommerce) has already appended it to $item_data.
		 *
		 * @since  x.x.x
		 * @access public
		 *
		 * @param  array $item_data Formatted display items built by woocommerce_get_item_data hooks.
		 * @param  array $cart_item Full cart item array.
		 *
		 * @return array $item_data with any esig_product_agreement entry removed.
		 */
		public function hide_agreement_from_cart_display( $item_data, $cart_item ) {
			foreach ( $item_data as $index => $data ) {
				$key = isset( $data['name'] ) ? $data['name'] : ( isset( $data['key'] ) ? $data['key'] : '' );
				if ( self::PRODUCT_AGREEMENT === $key || is_array( $data['value'] ?? null ) ) {
		...
		/**
		 * Handle after-checkout contract creation for a given order.
		 *
		 * Clones the configured standalone document(s) for the order, saves the
		 * resulting doc list to order meta, and redirects the customer to sign.
		 *
		 * An idempotency guard at the top prevents duplicate contracts when
		 * multiple hooks (woocommerce_order_status_changed, template_redirect,
		 * block checkout) race to call this method for the same order ID.
		 *
		 * @since 1.8.1
		 * @param int|string $order_id WooCommerce order ID.
		 * @return false|void Returns false on early exit; void otherwise.
		 */
		public function esig_after_order_status( $order_id ) {

			// Idempotency guard: contracts have already been created for this
			// order (doc list saved to meta). Bail out to prevent duplicates
			// caused by concurrent status-change hooks or page-load races.
			if ( esig_woo_logic::is_after_checkout_enable( $order_id ) ) {
				return;
			}
			...
			foreach ( $order->get_items() as $item_id => $order_item ) {
				...
				$esig_agreement   = $order_item->get_meta( self::PRODUCT_AGREEMENT, true ) ?: array();
			...
			// Use the WC Order API so this write goes to the correct storage
			// backend under both classic orders and HPOS.
			if ( empty( $agreement_list ) && class_exists( 'YITH_Request_Quote' ) ) {
				$order->update_meta_data( 'woo_esig_agreement', 'no' );
				$order->save();
			}
		...
		public function esig_after_checkout_form( $order_id, $posted_data, $order ) {
			...
			if ( empty( $agreement_list ) && class_exists( 'YITH_Request_Quote' ) ) {
				$yith_legacy_order = wc_get_order( $order_id );
				if ( $yith_legacy_order ) {
					$yith_legacy_order->update_meta_data( 'woo_esig_agreement', 'no' );
					$yith_legacy_order->save();
				}
			}
		...
		public function esig_checkout_after( $order_id ) {
			...
			// Read woo_esig_agreement using the WC Order API for HPOS compatibility.
			$esig_checkout_order = wc_get_order( $order_id );
			$noWooAgreement      = $esig_checkout_order ? $esig_checkout_order->get_meta( 'woo_esig_agreement', true ) : '';
