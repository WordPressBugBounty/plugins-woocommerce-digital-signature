...
class esig_woo_logic {
	...
	/**
	 * Get agreement id from a WooCommerce session.
	 *
	 * Returns false immediately when the Global Agreement setting is disabled so
	 * that stale cart session data from a previously-enabled state does not
	 * trigger a checkout redirect.
	 *
	 * @param string $is_true Agreement logic value to match ('before_checkout' or 'after_checkout').
	 * @return int|false Agreement ID, or false if not applicable.
	 */
	public static function get_global_doc_id_from_session( $is_true ) {
		if ( ! self::is_global_agreement_enabled() ) {
			return false;
		}
	...
	public static function saveMetaDoclist( $order_id, $doc_list ) {
		...
		$order->update_meta_data( '_esig_after_checkout_doc_list', json_encode( $doc_list ) );
		$order->save();
	}
	...
	public static function save_after_checkout_doc_list( $order_id, $doc_list ) {
		...
		self::saveMetaDoclist( $orderId, $docList );
	}
	...
	public static function get_after_checkout_doc_list( $order_id ) {
		...
		$listDoc = $order->get_meta( '_esig_after_checkout_doc_list', true );

		$listDoc  = str_replace( '\\', '', $listDoc );
	...
	public static function orderDetails( $orderId ) {
		...
		// Use wc_get_order() so this works with both classic and HPOS order storage.
		$order = wc_get_order( $orderId );
	...
	public static function clone_document( $sadDocId, $orderId ) {
		...
		$yith_quote_agreement = $order->get_meta( 'yith_quote_agreement_generated', true );
		...
		$data = self::orderDetails( $orderId );
	...
	public static function yith_quote_agreement( $order_id, $logic_type ) {
		...
		// Use the WC Order API so reads/writes work with both classic and HPOS storage.
		$order = wc_get_order( $order_id );
		if ( ! $order ) {
			return false;
		}
		...
		foreach ( $items as $item ) {
			...
			if ( self::is_signature_required( $product_id ) ) {

				$agreement_id = self::get_agreement_id( $product_id );

				$agreement_logic = self::get_agreement_logic( $product_id );

				if ( $agreement_logic == 'after_checkout' ) {
					$agreement_id                         = self::clone_document( $agreement_id, $order_id );
		...
		$global_esig_logic = self::get_global_logic();
		if ( self::is_global_agreement_enabled() && $global_esig_logic == 'after_checkout' ) {
			$global_agreement_id                  = self::get_global_agreement_id();
			$agreement_id                         = self::clone_document( $global_agreement_id, $order_id );
			$yith_agreement_list[ $agreement_id ] = 'no';
			// update_post_meta($order_id,'have_esig_agreement',esig_woo_logic::get_global_agreement_id());
		}

		if ( ! empty( $yith_agreement_list ) ) {
			$order->update_meta_data( 'yith_quote_agreement_generated', 'yes' );
			$order->save();
